...
XCBShow::XCBShow(){
	...
	std::cout<< "XCBSHow try to get a avaiable screen\r\n"<<std::endl;
	...
	if (adaptors == NULL){
		binit = false;
		xcb_disconnect(conn);
        std::cout<< "XCBSHow no avaiable adaptors avaiable"<<std::endl;
		return;		
	}
			 ...
			 xcb_xv_adaptor_info_next (&it))
	{
		...
		for (const xcb_xv_image_format_info_t *f =
					 ...
					 f < f_end; f++) {
                 std::cout<< "f-type="<<f->type<<std::endl;
				 if (f->type == XCB_XV_IMAGE_FORMAT_INFO_TYPE_YUV){
                     std::cout<< "Found avaiable YUV format display"<<std::endl;
					 bfound = true;
					 fid=f->id;
					 break;
				 }
	...
	if(fvisual==NULL){
        std::cout<< "XCBSHow not able to find a valid display \r\n"<<std::endl;
		binit = false;
		if(fport>0)
...
void XCBShow::create_win(int id){
	std::string title = "Workload Balance Demo"+std::to_string(id);
