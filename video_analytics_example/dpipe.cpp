...
dpipe_create(int id, const char *name, int nframe, int maxframesize) {
	...
	pthread_cond_init(&dpipe->cond_input, NULL);
	...
	printf("dpipe: '%s' initialized, %d frames, framesize = %d\n",
...
dpipe_destroy(dpipe_t *dpipe) {
	...
	pthread_cond_destroy(&dpipe->cond_input);
	...
	if(dpipe->in != NULL) {
		...
		/*if((vbuf = dpipe->out) != NULL) {
			...
			dpipe->out_count--;
                  ...
                  //printf("\r\n waiting for new free input buffer: count:%d\r\n", dpipe->in_count);
		 
                if(start){
                   pthread_mutex_unlock(&dpipe->io_mutex);
		   usleep(500);
                //  pthread_cond_wait(&dpipe->cond, &dpipe->io_mutex);
                // pthread_cond_wait(&dpipe->cond_input, &dpipe->io_mutex);
		  goto again;
                }else{
                  if((vbuf = dpipe->out) != NULL) {
			dpipe->out = vbuf->next;
...
dpipe_put(dpipe_t *dpipe, dpipe_buffer_t *buffer) {
	...
	dpipe->in_count++;
	pthread_mutex_unlock(&dpipe->io_mutex);
        pthread_cond_signal(&dpipe->cond_input);
	return;
}
